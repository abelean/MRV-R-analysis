#Nichenet analysis using DP cells as recievers and cTECs as senders, Mock vs. MRV
#load libaries
library( ggplot2 )
library( cowplot )
library( dplyr ) 
...
scd <- subset(scd, lin_X3 %in% c('DP','cTEC'))
...
scd.condition@active.ident<- factor(scd.condition@active.ident, levels = c('DP','DC','mTEC','cTEC'))##Add levels here
...
plot <- DotPlot(scd.condition, features = rev(colnames(lr_network_top_matrix_strict)), cols = "RdYlBu",
                scale.min = 0, scale.max = 50) + coord_flip() +
